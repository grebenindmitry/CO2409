...
//
// Creating geometry using index buffers
//
// Follows on from previous tutorials
//--------------------------------------------------------------------------------------
#include <windows.h>
#include <d3d10.h>
#include <d3dx10.h>
...
// and a colour for the vertex. The colours will be blended across the model.
struct SimpleVertex
{
...
HRESULT InitDevice()
{
	...
	hr = D3DX10CreateEffectFromFile(L"IndexBuffer.fx", NULL, NULL, "fx_4_0", dwShaderFlags, 0,
...
HRESULT InitGeometry()
{
	...
	//-----------------------------------
	// Define the vertex/index data

	// An array of vertices for the geometry we wish to render. Uses the structure at top of the file.
	// Now we are using an index buffer, we just list each vertex in the model once, with no duplication
	//
	SimpleVertex vertices[] =
	{
		D3DXVECTOR3(-1.0f, -1.0f, -1.0f), D3DXVECTOR4(1.0f, 0.1f, 0.1f, 0.0f),
		D3DXVECTOR3(-1.0f, 1.0f, -1.0f), D3DXVECTOR4(1.0f, 0.2f, 0.2f, 0.0f),
		D3DXVECTOR3(1.0f, -1.0f, -1.0f), D3DXVECTOR4(1.0f, 0.3f, 0.3f, 0.0f),
		D3DXVECTOR3(1.0f, 1.0f, -1.0f), D3DXVECTOR4(1.0f, 0.4f, 0.4f, 0.0f),
		D3DXVECTOR3(1.0f, -1.0f, 1.0f), D3DXVECTOR4(1.0f, 0.5f, 0.5f, 0.0f),
		D3DXVECTOR3(1.0f, 1.0f, 1.0f), D3DXVECTOR4(1.0f, 0.6f, 0.6f, 0.0f),
		D3DXVECTOR3(-1.0f, -1.0f, 1.0f), D3DXVECTOR4(1.0f, 0.7f, 0.7f, 0.0f),
	...
	// of vertices define a single triangle. This is just the front square (of a cube that you will build as an exercise)
	//
	DWORD indices[] =
	{
		3,1,5,7,6,1,0,3,2,5,4,6,2,0
	};
	...
	//-----------------------------------------------
	// Create vertex & index buffers from data above
	...
	bd.ByteWidth = sizeof(vertices);   // Indicate the size of the vertex buffer
	bd.CPUAccessFlags = 0;
	bd.MiscFlags = 0;
	D3D10_SUBRESOURCE_DATA InitData;
	InitData.pSysMem = vertices;         // The initial data for the vertex buffer - i.e. copy the array above into it
	hr = g_pd3dDevice->CreateBuffer(&bd, &InitData, &g_pVertexBuffer);
	if (FAILED(hr))
		return hr;

	// Set vertex buffer - tell D3D that the next vertex data to be rendered is in the vertex buffer we just created
	UINT stride = sizeof(SimpleVertex); // Size of a single vertex
	UINT offset = 0;
	g_pd3dDevice->IASetVertexBuffers(0, 1, &g_pVertexBuffer, &stride, &offset);


	// Create an "index buffer" to hold the indices on the GPU - exactly the same process as above for the vertex buffer
	bd.Usage = D3D10_USAGE_DEFAULT;
	...
	// Set index buffer - tell D3D that the next index data to be rendered is in the index buffer we just created
	g_pd3dDevice->IASetIndexBuffer(g_pIndexBuffer, DXGI_FORMAT_R32_UINT, 0);


	// Set primitive topology - tell D3D that the vertex/index buffer hold a triangle list, i.e. each triplet of indices should be
	// connected into triangles. There are other topologies, the most common being a triangle strip
...
//--------------------------------------------------------------------------------------
// Clean up the objects we've created when quiting
//--------------------------------------------------------------------------------------
void CleanupDevice()
{
...
// Update the world matrix of the model. This is done every frame, just before each render, allowing us to control or
// animate the model. The float parameter is how long in seconds has passed since the last frame. This can (optionally)
// be used to decide how much movement/rotation to perform
void UpdateModel1(float updateTime)
{
	// Update global variables (look at top of file) that control the model matrices
	if (KeyHeld(Key_W))
	{
		ModelXRot += 4.0f * updateTime; // Use of updateTime (time passed since last frame) makes movement same speed
										// regardless of speed of machine - in this case 4.0 radians per second
	}
	else if (KeyHeld(Key_S))
	{
		ModelXRot -= 4.0f * updateTime;
	}
	if (KeyHeld(Key_A))
	{
		ModelYRot += 4.0f * updateTime;
	}
	...
	// Make the model's world matrix by combining three rotation matrices, only the Y rotation is under user control at first
	D3DXMATRIX matrixXRot;
	D3DXMATRIX matrixYRot;
	D3DXMATRIX matrixZRot;
	D3DXMatrixRotationX(&matrixXRot, ModelXRot);
	D3DXMatrixRotationY(&matrixYRot, ModelYRot);
	D3DXMatrixRotationZ(&matrixZRot, 0.0f);
	g_WorldMatrix1 = matrixZRot * matrixXRot * matrixYRot;

	// Pass the model's world matrix to the shader (on the GPU). We'll see more about this later
...
void Render()
{
	...
	UpdateModel1(updateTime);

	// Render the model. We have already told D3D the format of the geometry, and where to find it (in the vertex/index buffer). We prepared
	...
	// detail later, so for now the only key line is the one containing "DrawIndexed"
	D3D10_TECHNIQUE_DESC techDesc;
	g_pTechnique->GetDesc(&techDesc);
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		g_pTechnique->GetPassByIndex(p)->Apply(0);
		g_pd3dDevice->DrawIndexed(14, 0, 0); // Draw the first 6 indexed vertices (2 triangles in a triangle list), 
											  // starting at the beginning of the index list (second parameter 0) and with
											  // no offset (third parameter 0 - an advanced topic)
											  // Note this line is different from previous weeks as we're using an index buffer
	}
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow)
{
	...
	if (FAILED(InitDevice()) || FAILED(InitGeometry()))
	{
		CleanupDevice();
		return 0;
	}
...
HRESULT InitWindow(HINSTANCE hInstance, int nCmdShow)
{
	...
	g_hWnd = CreateWindow(L"TutorialWindowClass", L"Direct3D 10: Index Buffers", WS_OVERLAPPEDWINDOW,
