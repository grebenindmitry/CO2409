...
//DG: A generic clamp function, that outputs the minimum or maximum value if the value is out of bounds (both bounds are inclusive)
//DG: e.g., Clamp(100, 0, 50) == 50, Clamp(-100, 0, 50) == 0, Clamp(25, 0, 50) == 25
//DG: works with any structure with > and < operators implemented
template<class T>
...
{
	//DG: copy all the pixels of the bitmap onto the viewport
...
{
	//DG: draw all non-transparent pixels of the bitmap onto the viewport
	for (int x = 0; x < SrcBitmap->Width; x++) {
		for (int y = 0; y < SrcBitmap->Height; y++) {
...
void SpritePlotterForm::DrawSpriteAdd( int X, int Y, Bitmap^ SrcBitmap )
{
	...
	for (int x = 0; x < SrcBitmap->Width; x++) {
		for (int y = 0; y < SrcBitmap->Height; y++) {
			...
			//DG: the final color is calculated by adding R, G, B of src to viewport, clamping the value to [0-255]
			finalColor = Color().FromArgb(
...
void SpritePlotterForm::DrawSpriteMultiply( int X, int Y, Bitmap^ SrcBitmap )
{
	...
	for (int x = 0; x < SrcBitmap->Width; x++) {
		for (int y = 0; y < SrcBitmap->Height; y++) {
			...
			//DG: the final color is calculated by multiplying R, G, B of src to viewport, and dividing by 255
			//DG: to get back to [0-255]
			finalColor = Color().FromArgb(
