...
// Cube rotations
D3DXVECTOR3 CubeRot = D3DXVECTOR3(0.f, 0.f, 0.f);
D3DXVECTOR3 CubeLocalPos = D3DXVECTOR3(0.f, 0.f, 0.f);
D3DXVECTOR3 CamPos = D3DXVECTOR3(0.f, 0.f, -5.f);
D3DXVECTOR3 CubeScale = D3DXVECTOR3(1.f, 1.f, 1.f);
...
void UpdateCube1(float updateTime)
{
	...
	if (KeyHeld(Key_A)) CubeRot.y += updateTime;
	if (KeyHeld(Key_D)) CubeRot.y -= updateTime;
	if (KeyHeld(Key_W)) CubeRot.z += updateTime;
	if (KeyHeld(Key_S)) CubeRot.z -= updateTime;
	if (KeyHeld(Key_Q)) CubeRot.x += updateTime;
	if (KeyHeld(Key_E)) CubeRot.x -= updateTime;

	//DG: controls for position
	if (KeyHeld(Key_J)) CubeLocalPos.x -= updateTime;
	if (KeyHeld(Key_L)) CubeLocalPos.x += updateTime;
	if (KeyHeld(Key_I)) CubeLocalPos.z += updateTime;
	if (KeyHeld(Key_K)) CubeLocalPos.z -= updateTime;
	if (KeyHeld(Key_U)) CubeLocalPos.y += updateTime;
	if (KeyHeld(Key_O)) CubeLocalPos.y -= updateTime;

	//DG: controls for scaling
	if (KeyHeld(Key_1) && CubeScale.x > 0) CubeScale.x -= updateTime;
	if (KeyHeld(Key_2) && CubeScale.x < D3D10_FLOAT32_MAX) CubeScale.x += updateTime;

	D3DXMATRIX matrixRot;
	D3DXMATRIX matrixPos;
	D3DXMATRIX matrixScale;

	D3DXMatrixRotationYawPitchRoll(&matrixRot, CubeRot.y, CubeRot.z, CubeRot.x);
	D3DXMatrixScaling(&matrixScale, CubeScale.x, CubeScale.y, CubeScale.z);

	//DG: first scale, then rotate
	g_WorldMatrix1 = matrixScale * matrixRot;

	//DG: translate the cube along the local axes
	D3DXMatrixTranslation(&matrixPos,
		g_WorldMatrix1._11 * CubeLocalPos.x + g_WorldMatrix1._21 * CubeLocalPos.y + g_WorldMatrix1._31 * CubeLocalPos.z,
		g_WorldMatrix1._12 * CubeLocalPos.x + g_WorldMatrix1._22 * CubeLocalPos.y + g_WorldMatrix1._32 * CubeLocalPos.z,
		g_WorldMatrix1._13 * CubeLocalPos.x + g_WorldMatrix1._23 * CubeLocalPos.y + g_WorldMatrix1._33 * CubeLocalPos.z);
...
{
	//DG: set the cube's position
	D3DXMatrixTranslation(&g_WorldMatrix2, 0.0f, 0.0f, 10.0f);

	// Pass the cube's world matrix to the shader (on the GPU). We'll see more about this later
	g_pWorldVariable1->SetMatrix((float*)&g_WorldMatrix2);
}
...
{
	//DG: Camera controls
	if (KeyHeld(Key_Up)) CamPos.y += updateTime;
	if (KeyHeld(Key_Down)) CamPos.y -= updateTime;
	if (KeyHeld(Key_Right)) CamPos.x += updateTime;
	if (KeyHeld(Key_Left)) CamPos.x -= updateTime;

	//DG: create a lookat matrix at CamPos, looking at the cube
	D3DXMatrixLookAtLH(&g_ViewMatrix, &CamPos, &D3DXVECTOR3(g_WorldMatrix1._41, g_WorldMatrix1._42, g_WorldMatrix1._43), &D3DXVECTOR3(0.0f, 1.0f, 0.0f));
